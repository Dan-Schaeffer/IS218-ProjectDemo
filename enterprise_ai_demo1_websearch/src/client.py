...
# --- Chat client for Code Explainer -----------------------------------------
import os
...
class ChatClient:
    """
    Minimal chat-capable client used by code_explainer_service.
    Exposes:
      - .default_model
      - .chat(messages=[{role,content},...], model=None, temperature=..., max_tokens=...)
    Returns an OpenAI-like dict: {"choices":[{"message":{"content": "..."} }]}
    """
    def __init__(self, api_key: str | None = None, default_model: str | None = None):
        self.api_key = api_key or os.getenv("OPENAI_API_KEY") # pragma: no cover
        if not self.api_key: # pragma: no cover
            raise ValueError("OPENAI_API_KEY not found for ChatClient") # pragma: no cover
        if OpenAI is None: # pragma: no cover
            raise RuntimeError("OpenAI SDK not installed. Run: pip install openai>=1.51.0") # pragma: no cover

        # Reuse your repo’s default if you like; fallback is gpt-4o-mini
        self.default_model = default_model or os.getenv("OPENAI_MODEL", "gpt-4o-mini") # pragma: no cover
        self._client = OpenAI(api_key=self.api_key) # pragma: no cover

    def chat(self, *, messages, model=None, temperature=0.2, max_tokens=800):
        use_model = model or self.default_model # pragma: no cover
        # Use Chat Completions for broad compatibility
        resp = self._client.chat.completions.create( # pragma: no cover
            model=use_model, # pragma: no cover
            messages=messages, # pragma: no cover
            temperature=temperature, # pragma: no cover
            max_tokens=max_tokens, # pragma: no cover
        ) # pragma: no cover
        # Normalize to the dict shape code_explainer_service._extract_content expects
        content = resp.choices[0].message.content if resp.choices else "" # pragma: no cover
        return {"choices": [{"message": {"content": content}}]} # pragma: no cover

def get_client() -> ChatClient:
    """Factory so _get_default_client() can find us."""
    return ChatClient() # pragma: no cover
